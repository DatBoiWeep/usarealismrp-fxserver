...
RegisterNetEvent("cdev_vehicleshop:chargeTestDriveFee", function()
...
ResourceAPI.Queries = {
    ...
    InsertShop = function(name, logo, blip, blipColor, blipLocation, tablets, showroom, spawns, management, settings,
                          vaultMoney)
        clib.db.Insert([[
            INSERT INTO cdev_vshop_shops(id, name, logo, blip, blipColor, blipLocation, tablets, showroom, spawns, management, settings, vault)
            VALUES(NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ]], {
            name,
            logo,
    ...
    UpdateShop = function(name, logo, blip, blipColor, blipLocation, tablets, showroom, spawns, management, vaultMoney,
                          settings, id)
        clib.db.Update([[
            UPDATE cdev_vshop_shops
            SET name = ?, logo = ?, blip = ?, blipColor = ?, blipLocation = ?, tablets = ?, showroom = ?, spawns = ?, management = ?, vault = ?, settings = ?
            WHERE id = ?
        ]], {
            name,
            logo,
    ...
    UpsertDefaultVehicle = function(vehicleModel, category, price)
        ...
        if vehicle and #vehicle > 0 then
            ...
            clib.db.Insert([[
                ...
                vehicleModel,
                price,
                'https://static.cdev.shop/resources/vehicle-shop/thumbnails/' .. vehicleModel .. '.png',
                category,
            })
        end
    end,
