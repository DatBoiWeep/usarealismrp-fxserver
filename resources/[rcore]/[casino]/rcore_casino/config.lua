Config = {
    ...
    -- Map
	-- 1: for DLCiplLoader, 2: Gabz Casino, 3: NoPixel Casino, 4: k4mb1 casino, 5: GTA:O Interior (rcore_casino_interior), 6: Underground Casino PALETO, 7: K4MB1 old enterable casino (casinointerior & casinoexterior)
    ...
    IT_MAIN_EVENT_RACE_MAX_BET = 15000, -- max. bet for the main event race
    ...
    IT_LOCAL_RACE_MAX_BET = 7000, -- max. bet for the local race
    ...
    LUCKY_WHEEL_PAY_TO_SPIN = 3000, -- set price for the spin (chips), set 0 for free spins, or set to existing inventory name, for example LUCKY_WHEEL_PAY_TO_SPIN = "wheel_ticket" to pay with an inventory item
    LUCKY_WHEEL_CAR_ONE_WINNER = false, -- set to true, if only one player can win the podium vehicle. the vehicle will disapear from the podium after someone wins it.
    ...
    CASINO_AMBIENT_PEDS_DENSITY = 1, -- 1: a few, 2: medium, 3: all peds
    ...
    --
    JOB_PODIUMCAR_OWNERSHIP_CHECK = false,
    JOB_PODIUMCAR_OWNERSHIP_DELETE_ORIGINAL = false,
    ...
    -- Other Resources
    EnableGuidebookIntegration = true, -- https://store.rcore.cz/package/5041989
...
Framework = {
    ...
    Active = 4, -- Choose 1 for ESX, 2 for QBCore, 3 for Standalone, 4 for Custom, 5 for auto detect
