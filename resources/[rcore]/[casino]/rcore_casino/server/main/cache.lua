...
function Cache:PlayerOwnsVehicle(identifier, plate)
    ...
    if Framework.Active == 4 then
        local charData = exports.essentialmode:getDocument("characters", identifier)
        for i = 1, #charData.vehicles do
            if plate == charData.vehicles[i] then
                return true
            end
        end
        return false
...
function Cache:GiveVehicle(playerId, vehicleProps)
    ...
    if Framework.Active == 4 then
        ...
        local vehInfo = exports["usa_carshop"]:GetVehicleByHashNumber(vehicleProps.model)
        if not vehInfo then
            vehInfo = {}
            vehInfo.make = "unknown"
            vehInfo.model = "unknown"
            vehInfo.hash = vehicleProps.model
            vehInfo.price = 0
            vehInfo.storageCapacity = 100
        end
        local vehicle = {
        ...
        -- notify
        TriggerClientEvent("usa:notify", playerId, "You won a car!!", "INFO: " .. "You won a " .. vehicleProps.podiumName .. "! Congratulations!! Check your garage!!")
        return
    end
...
local randomLuckyWheelPrizeVeh = {
    modSeats = -1,
    engineHealth = 1000,
    modExhaust = -1,
    modRightFender = -1,
...
randomLuckyWheelPrizeVeh.plate = nil
randomLuckyWheelPrizeVeh.podiumName = chosenRandomModel
randomLuckyWheelPrizeVeh.model = GetHashKey(chosenRandomModel)
Cache:UpdateSetting("PodiumPriceProps", json.encode(randomLuckyWheelPrizeVeh))
TriggerClientEvent("Casino:PodiumReplace", -1, -1)